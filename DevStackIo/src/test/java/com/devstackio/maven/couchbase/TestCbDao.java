...
/**
 * tests to validate CbDao functionality
 * pre-req : 
 *	- couchbase bucket setup ( name / password )
*		bucket name "default" with no password
 * @author devstackio
 */
public class TestCbDao {
	 ...
	 */
	private String createMockContractEntity( String contract, boolean tosession ) {
	...
	/**
	 * Test create and read methods of EntityCbDao (abstract super class)
 tests abstract convert (overwritten) method of ContractCbDao.
	...
	public void testCreateAndRead() {
		...
		try {
			docId = this.createMockContractEntity( contract, false);
 ...
 tests abstract convert (overwritten) method of ContractCbDao.
	...
	public void testCreateAndReadSession() {
		...
		try {
			docId = this.createMockContractEntity( contract, true);
			System.out.println("saved doc as : " + docId);
